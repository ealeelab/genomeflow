...
credentialFileName = os.getenv("CREDENTIAL_FILE_NAME")
...
try:
    ...
    print("execute network speed monitoring daemon")
    networkLogger=subprocess.Popen([workPath+"/checkNetworkSpeed.sh"])
    ...
    #os.system('export PATH=/app/sratoolkit.2.10.8-centos_linux64/bin:$PATH')
    os.system('rm -rf /root/.ncbi')
    os.system('mkdir /root/.ncbi')
    os.system('cp user-settings.mkfg /root/.ncbi/')

    cmd = 'mkdir ' + os.path.join(workPath,sampleName)
    os.system(cmd)
    #os.system('cd '+sampleName)
    os.chdir(os.path.join(workPath,sampleName))

    cmd = 'prefetch ' + sampleName
    #cmd = 'fasterq-dump -p --split-files '+sampleName
    print(cmd)
    os.system(cmd)

    # Generate Fastqfile

    cmd = 'fastq-dump -I --split-files --log-level 5 '+sampleName+'/'+sampleName+'*.sra'
    print(cmd)
    os.system(cmd)
    print('gpiz fastq file')
    os.system('pigz --best ' + sampleName+'*_1.fastq')
    os.system('pigz --best ' + sampleName+'*_2.fastq')
    os.system('mv '+sampleName+'*_1.fastq.gz '+sampleName+'_1.fastq.gz')
    os.system('mv '+sampleName+'*_2.fastq.gz '+sampleName+'_2.fastq.gz')
    #os.system('cd ..')
    os.chdir(workPath)
    # User custom command for download end
    ...
    cmd = 'gtex_pipeline.sh ' \
            ' ' + sampleName + \
            ' ' + cores + \
            ' ' + sampleOutputPath +' 2>&1 | tee ' + sampleOutputPath + '/job.log'
    ...
    if finalize is True:
        ...
        if (len(glob.glob(os.path.join(sampleOutputPath,"job.log"))) > 0):
            ...
            uploadFileToBucket(bucketName,queue,sampleName,"networkSpeed.log",os.path.join(workPath,"networkSpeed.log"))
            uploadFileToBucket(bucketName,queue,sampleName,"worker.log",os.path.join(workPath,"worker.log"))
