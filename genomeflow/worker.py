#!/usr/bin/env python
# Kubernetes Pod worker for task processing
...
try:
    ...
    cmd = 'referenceDownload.sh ' + ref_bucket+'/'+ref_prefix + ' ' + refDir
    print(cmd)
    os.system(cmd)
    ...
    cmd = 'sampleDownload.sh ' + sampleName + ' ' + os.path.join(workPath,sampleName)
    print(cmd)
    os.system(cmd)
    ...
    # Additional Program (TELocal, TETranscript, L1EM)
    cmd = 'additionalPrograms.sh ' \
        ' ' + sampleName + \
        ' ' + refDir + '/additionalPrograms' \
        ' ' + os.path.join(workPath,sampleName) + \
        ' /additionalResults' + \
        ' ' + queue + \
        ' ' + cores
